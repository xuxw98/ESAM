...
def export_one_scan(scan_name):    
    ...
    aligned_mesh_vertices, instance_labels, bboxes, label_map, object_id_to_label = \
        export(mesh_file, agg_file, seg_file, meta_file, 'meta_data/scannetv2-labels.combined.tsv', scannet200=False)
...
def process_cur_scan(cur_scan, mask_generator, split):
    ...
    np.save(os.path.join("axis_align_matrix", scan_name + ".npy"), axis_align_matrix)
    ...
    for frame_i, (rgb_map_name, \
        ...
        unaligned_xyz.astype(np.float32).tofile(
            os.path.join("points/%s" % scan_name, "%s.bin" % (20*frame_i)))
        sem.astype(np.int64).tofile(
            os.path.join("semantic_mask/%s" % scan_name, "%s.bin" % (20*frame_i)))
        ins.astype(np.int64).tofile(
            os.path.join("instance_mask/%s" % scan_name, "%s.bin" % (20*frame_i)))
