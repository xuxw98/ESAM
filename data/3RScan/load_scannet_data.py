...
def extract_bbox(mesh_vertices, object_id_to_segs, object_id_to_label_id,
                 instance_ids):
    num_instances = len(np.unique(instance_ids)) - 1
    instance_bboxes = np.zeros((num_instances, 7))
    for idx, obj_id in enumerate(object_id_to_segs):
...
def export(mesh_file,
    ...
    #                                        axis=1)
    aligned_mesh_vertices = mesh_vertices.copy()
    ...
    return mesh_vertices, label_ids, instance_ids, unaligned_bboxes, \
        aligned_bboxes, object_id_to_label_id
